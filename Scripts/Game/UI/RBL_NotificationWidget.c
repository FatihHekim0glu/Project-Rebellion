...
// ============================================================================
// Preset notification types for easy use
// ============================================================================
class RBL_Notifications
{
	static const int PRIORITY_LOW = 0;
	static const int PRIORITY_NORMAL = 5;
	static const int PRIORITY_HIGH = 10;
	static const int PRIORITY_CRITICAL = 15;
	
	// Zone notifications
	static void ZoneCaptured(string zoneName)
	{
		RBL_UIManager.GetInstance().GetNotifications().ShowNotificationWithPriority(
			zoneName + " CAPTURED!",
	...
	static void ZoneLost(string zoneName)
	{
		RBL_UIManager.GetInstance().GetNotifications().ShowNotificationWithPriority(
			zoneName + " LOST!",
			RBL_UIColors.COLOR_ACCENT_RED,
			4.0,
			PRIORITY_CRITICAL
		);
	}
	
	static void ZoneUnderAttack(string zoneName)
	{
		RBL_UIManager.GetInstance().GetNotifications().ShowNotificationWithPriority(
			zoneName + " is under attack!",
	...
	static void MoneyReceived(int amount, string reason)
	{
		RBL_UIManager.GetInstance().GetNotifications().ShowNotification(
			"+" + RBL_UIStrings.FormatMoney(amount) + " - " + reason,
			RBL_UIColors.COLOR_ACCENT_GREEN,
			2.5
		);
	}
	
	static void MoneySpent(int amount, string reason)
	{
		RBL_UIManager.GetInstance().GetNotifications().ShowNotification(
			"-" + RBL_UIStrings.FormatMoney(amount) + " - " + reason,
			RBL_UIColors.COLOR_ACCENT_AMBER,
	...
	static void QRFIncoming(string qrfType)
	{
		RBL_UIManager.GetInstance().GetNotifications().ShowNotificationWithPriority(
			"QRF INCOMING: " + qrfType,
			RBL_UIColors.COLOR_ACCENT_RED,
			5.0,
			PRIORITY_CRITICAL
		);
	...
	static void MissionStarted(string missionName)
	{
		RBL_UIManager.GetInstance().GetNotifications().ShowNotificationWithPriority(
			"MISSION: " + missionName,
			RBL_UIColors.COLOR_ACCENT_BLUE,
			4.0,
			PRIORITY_HIGH
		);
	}
	
	static void MissionCompleted(string missionName)
	{
		RBL_UIManager.GetInstance().GetNotifications().ShowNotificationWithPriority(
	...
	static void GameSaved()
	{
		RBL_UIManager.GetInstance().GetNotifications().ShowNotification(
			"Game saved",
			RBL_UIColors.COLOR_TEXT_SECONDARY,
			2.0
		);
	}
	...
	static void Show(string message)
	{
		RBL_UIManager.GetInstance().GetNotifications().ShowNotification(
			...
			RBL_UITiming.TOAST_DURATION
		);
	}
	
	static void ShowColored(string message, int color)
	{
		RBL_UIManager.GetInstance().GetNotifications().ShowNotification(
			message,
			color,
