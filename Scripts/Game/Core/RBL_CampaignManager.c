...
class RBL_CampaignManager : GenericEntity
{
	...
	protected const float AGGRESSION_DECAY_RATE = 0.5;
	...
	protected void InitializeWarLevelThresholds()
	{
		...
		m_aWarLevelThresholds.Insert(20);
		m_aWarLevelThresholds.Insert(50);
		m_aWarLevelThresholds.Insert(100);
		m_aWarLevelThresholds.Insert(175);
		m_aWarLevelThresholds.Insert(275);
		m_aWarLevelThresholds.Insert(400);
		m_aWarLevelThresholds.Insert(550);
		m_aWarLevelThresholds.Insert(750);
		m_aWarLevelThresholds.Insert(1000);
	}
	...
	{
		float decay = AGGRESSION_DECAY_RATE * (timeSlice / 60.0);
	...
	protected void ProcessResourceTick()
	{
		...
		{
			totalMoney += playerZones[i].CalculateResourceIncome();
			totalHR += playerZones[i].CalculateHRIncome();
		}
