...
class RBL_CaptureManager
{
	...
	protected const float CAPTURE_RATE = 2.0; // Progress per second with 1 player
	protected const float CAPTURE_RATE_BONUS = 0.5; // Extra per additional player
	...
	protected void ProcessCapture(RBL_VirtualZone zone, ERBLFactionKey capturingFaction, int netStrength)
	{
		...
		float progressGain = CAPTURE_RATE + (CAPTURE_RATE_BONUS * (netStrength - 1));
	...
	protected void DecayCaptureProgress(string zoneID)
	{
		...
		currentProgress -= CAPTURE_RATE * 0.5;
		if (currentProgress < 0)
			currentProgress = 0;
